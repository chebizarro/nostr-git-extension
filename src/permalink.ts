function computeGitHubPermalink(): string | null {
	...
	switch (section) {
	  case "blob": {
		...
		return `${origin}/${owner}/${repo}/blob/${ref}/${filePath.join("/")}${hash}`;
	  }
  
	  case "tree": {
		const [ref, ...folderPath] = rest;
		return `${origin}/${owner}/${repo}/tree/${ref}/${folderPath.join("/")}${hash}`;
	  }
	  ...
	  default:
		// fallback
		const permalinkAnchor = document.querySelector<HTMLAnchorElement>('a.js-permalink-shortcut');
